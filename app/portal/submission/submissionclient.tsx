...
export default function SubmissionClient() {
  ...
  function handleFiles(key: string, selected: FileList | null, multiple: boolean) {
    ...
    setFiles((prev) => ({ ...prev, [key]: multiple ? [...(prev[key] ?? []), ...arr] : [arr[0]] }));
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const uploadRes = await fetch("/api/submissions/upload-photos", { method: "POST", body: fd });
      const uploadJson = await uploadRes.json();
      if (!uploadRes.ok) throw new Error(uploadJson?.error || "Photo upload failed");
      const photoUrls = uploadJson.urls;
  ...
  return (
    <div className="mt-8 flex flex-col gap-8">
      ...
      {error && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-5 py-3 text-sm text-red-700">{error}</div>
      )}

      <div className="flex items-center justify-between rounded-2xl bg-blue-50 px-6 py-4 ring-1 ring-blue-200">
        <p className="text-sm text-blue-900">
          {totalFiles === 0 ? "No files selected yet." : `${totalFiles} file${totalFiles !== 1 ? "s" : ""} ready to submit.`}
        </p>
        <button
        ...
        >
          {busy && <span className="mr-2 inline-block h-3 w-3 animate-spin rounded-full border-2 border-black/30 border-t-black" />}
          {STAGE_LABEL[stage]}
        </button>
      </div>
    </div>
...
function UploadCard({ slot, selectedFiles, onFiles, onRemove, disabled }: {
  slot: UploadSlot; selectedFiles: File[]; onFiles: (f: FileList | null) => void; onRemove: (i: number) => void; disabled?: boolean;
}) {
    ...
    <section className="rounded-3xl bg-white p-8 shadow-sm ring-1 ring-blue-100">
      ...
      <div className="mt-6 rounded-2xl bg-blue-50 p-6 ring-1 ring-blue-100">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <label className="text-sm font-semibold text-blue-950">{slot.multiple ? "Select images" : "Select an image"}</label>
          ...
          <input ref={inputRef} type="file" className="hidden" multiple={slot.multiple} accept="image/png,image/jpeg,image/webp" onChange={(e) => onFiles(e.target.files)} />
        </div>
        ...
        {selectedFiles.length > 0 && (
          <ul className="mt-4 flex flex-col gap-2">
            {selectedFiles.map((f, i) => (
              <li key={i} className="flex items-center justify-between rounded-xl bg-white px-4 py-2 text-sm text-blue-950 ring-1 ring-blue-100">
                <span className="truncate max-w-[200px]">{f.name}</span>
                <button onClick={() => onRemove(i)} disabled={disabled} className="ml-3 text-xs font-semibold text-red-400 hover:text-red-600 disabled:opacity-40">Remove</button>
              </li>
            ))}
          </ul>
        )}
