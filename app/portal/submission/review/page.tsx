...
export default function SubmissionReviewPage() {
  ...
  function toggleFlag(flag: FlagKey) {
    setExtraFlags((prev) => prev.includes(flag) ? prev.filter((f) => f !== flag) : [...prev, flag]);
  }
  ...
  if (!edited) {
    return <main className="mx-auto max-w-6xl px-6 py-10"><div className="rounded-3xl bg-white p-10 shadow-lg text-blue-900">Loading…</div></main>;
  }

  const hasChanges = JSON.stringify({
    tagNumber: edited.tagNumber, poleCondition: edited.poleCondition, padCondition: edited.padCondition,
    overviewNotes: edited.overviewNotes, baseNotes: edited.baseNotes, vegetationEncroachment: edited.vegetationEncroachment,
  }) !== JSON.stringify({
    tagNumber: data?.tagNumber, poleCondition: data?.poleCondition, padCondition: data?.padCondition,
    overviewNotes: data?.overviewNotes, baseNotes: data?.baseNotes, vegetationEncroachment: data?.vegetationEncroachment,
  });
    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="rounded-3xl bg-white p-10 shadow-lg">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-4xl font-extrabold tracking-tight text-blue-950">Submission Review</h1>
            <p className="mt-2 text-blue-900/70">Review and correct the AI's findings before confirming.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {autoFlags.includes("vegetation") && <FlagPill color="bg-green-100 text-green-800 ring-green-300" label="🌿 Vegetation" />}
            {extraFlags.includes("review") && <FlagPill color="bg-purple-100 text-purple-800 ring-purple-300" label="🔍 Review" />}
            {autoFlags.length === 0 && extraFlags.length === 0 && (
              <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-500 ring-1 ring-blue-100">No flags</span>
            )}
          </div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {(["tag", "overview", "base", "pad"] as const).map((cat) => (
            <div key={cat} className="rounded-2xl bg-blue-50 px-5 py-4 text-center ring-1 ring-blue-100">
              ...
              <p className="mt-1 text-xs font-semibold capitalize text-blue-700">{cat} photo{edited.fileCounts[cat] !== 1 ? "s" : ""}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-10 grid gap-8 md:grid-cols-2">
          <Field label="Tag Number" aiGenerated>
            <input type="text" value={edited.tagNumber} onChange={(e) => update("tagNumber", e.target.value)} className="input" />
          </Field>
          ...
          <Field label="Vegetation Encroachment" aiGenerated>
            <div className="flex gap-3">
              {[true, false].map((val) => (
                <button key={String(val)} onClick={() => update("vegetationEncroachment", val)}
                  className={`flex-1 rounded-xl border py-3 text-sm font-bold transition-all ${
                    edited.vegetationEncroachment === val
                      ? val ? "border-green-400 bg-green-100 text-green-900" : "border-blue-400 bg-blue-100 text-blue-900"
          ...
          <Field label="Overview Notes" aiGenerated>
            <textarea rows={3} value={edited.overviewNotes} onChange={(e) => update("overviewNotes", e.target.value)} className="input resize-none" />
          </Field>
          <Field label="Base Notes" aiGenerated>
            <textarea rows={3} value={edited.baseNotes} onChange={(e) => update("baseNotes", e.target.value)} className="input resize-none" />
          </Field>
        </div>

        <div className="mt-10">
          <h2 className="text-sm font-bold uppercase tracking-wider text-blue-500 mb-3">Inspector Flags</h2>
            ...
            {(Object.entries(FLAG_META) as [FlagKey, typeof FLAG_META[FlagKey]][]).map(([key, meta]) => (
              <button key={key} onClick={() => toggleFlag(key)}
                className={`rounded-xl border px-4 py-2 text-sm font-semibold ring-1 transition-all ${
                  extraFlags.includes(key) ? meta.color : "border-blue-200 bg-blue-50 text-blue-900 ring-blue-100 hover:bg-blue-100"
                }`}>
        ...
        {hasChanges && <p className="mt-6 text-sm font-semibold text-yellow-600">✎ You have made changes from the AI's original output.</p>}
        {error && <div className="mt-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}

        <div className="mt-10 flex flex-wrap gap-4">
          <button onClick={() => router.push("/portal/submission")} disabled={saving || saved}
      ...
      <style>{`.input { width:100%; border-radius:0.75rem; border:1px solid #bfdbfe; background:#eff6ff; padding:0.75rem 1rem; font-size:0.875rem; color:#172554; outline:none; } .input:focus { border-color:#60a5fa; box-shadow:0 0 0 2px #dbeafe; }`}</style>
    </main>
  );
}
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        {aiGenerated && <span className="rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-blue-600">AI</span>}
      </div>
      {children}
    </div>
  );
